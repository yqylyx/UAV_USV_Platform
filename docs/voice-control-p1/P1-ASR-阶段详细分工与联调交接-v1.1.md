...
D1只做录音→本地识别→显示、编辑文字，不接意图大模型、不生成提案、不执行控制指令、不依赖算法实例与Unity就绪。本地语音识别固定接入算法仿真页面右侧“语音控制”面板，不设置独立页面或侧边栏入口。前端经Java调用独立Python ASR，不直接访问Python。Python独立环境/进程，不改算法Runner；Unity无新增功能，只做兼容。原P0手工控制独立保留。

首轮完成标准：进入算法仿真工作区但不生成场景、不启动算法实例，在右侧面板录一段中文，真实本地模型返回文字且可编辑；说“停止任务”也只显示文字，不执行任何动作。
...
| ID | 场景与标准 |
|---|---|
| D01 | 进入算法仿真工作区但不生成场景或启动算法实例；右侧面板localhost浏览器真实录音→Java→本地模型→可编辑文字 |
| D02 | 拒绝麦克风，提示明确，无空上传 |
| D03 | 静音422，无伪造成功文字 |
...
| D12 | 右侧面板内ASR与原P0手工、Unity兼容；不开独立ASR页面，ASR开关/重启不误停算法Runner |
...
在算法仿真页面右侧“语音控制”面板接入本地语音识别，不增加独立页面或导航；进入工作区即可使用，不要求生成场景或启动算法实例。ASR-only隐藏意图解析。指定Chrome/Edge版本及WebM/Opus，58秒软停止/尾帧处理/真实上限提示，动态CSRF与三标识，140秒等待和主动取消。完整显示500码点并编辑。首次提交10分钟手动同键原Blob恢复，遵守Retry-After；卸载/退出/换用户清理，迟到隔离。报告本次测试及构建，不用历史64项代替本轮结果。
...
维护公共契约/Schema/样例与内部适配；ADMIN/CSRF/输入及返回前权限，真实错误映射/ID校验。按1000条/30分钟/每分钟清理维护内存完整结果，并使用7天最小受理墓碑阻止Java重启后的旧键重复推理；已有键优先、终态失败也缓存、活跃不清理。上传10秒/处理120秒/剩余等待，不自动重试。部署同机Java/Python/浏览器，localhost代理与Cookie/CSRF对齐，密钥和音频不落盘。保持P0基线，不实现意图提案关联。
...
E1完整46项以配套规划v1.3第8节为权威表；D1只增加最小墓碑与重启防重复推理，完整结果持久化、五格式和60条统计仍待办。本文是最终分工与约定，不声称任何尚未执行的测试已通过。
