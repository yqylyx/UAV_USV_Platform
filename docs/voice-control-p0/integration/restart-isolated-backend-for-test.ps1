...
$repo=$env:P0_WORKSPACE
if(!$repo){throw 'P0_WORKSPACE is required'}
$localDir=$env:P0_ISOLATED_LOCAL_DIR
if(!$localDir){throw 'P0_ISOLATED_LOCAL_DIR is required'}
$evidence=$env:P0_GAP_EVIDENCE_DIR
if(!$evidence -or !(Test-Path -LiteralPath $evidence)){throw 'Existing P0_GAP_EVIDENCE_DIR is required'}
