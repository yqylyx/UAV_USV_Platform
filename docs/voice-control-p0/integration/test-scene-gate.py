...
try:
 ...
 status,b=call('/api/voice/commands/proposals',{'runtimeRef':ref,'runtimeGeneration':prep['runtimeGeneration'],'expectedContextVersion':context()['contextVersion'],'intent':'MISSION_START'})
 report['startProposal']={'httpStatus':status,'body':b}
 assert status==409 and b['code']=='SCENE_NOT_READY',(status,b)
 assert not rows(),'Unexpected execution created for blocked proposal'
 proposals=query("SELECT data_json FROM voice_proposal WHERE runtime_ref='"+ref+"'")
 assert not proposals,'Unexpected proposal created'
 report['proposalCountBeforeCleanup']=len(proposals)
 report['executionCountBeforeCleanup']=0
 report['presentation']='BLOCKED: actual scene not ready; no fake reports sent'
 report['result']='PASS_NEGATIVE_GATE; REAL_PRESENTATION_BLOCKED'
 print('START proposal: 409 SCENE_NOT_READY; proposals=0 executions=0',flush=True)
 # Cleanup uses the real manual STOP endpoint, not a presentation report.
 ok('/api/algorithm-runs/'+str(run)+'/stop',{})
 wait(lambda: context()['state']=='STOPPED')
 stopped=True
 report['cleanup']='Real manual STOP reached STOPPED'
