...
try {
 ...
 const repeated=await page.evaluate(async p=>{const csrf=(await(await fetch('/api/auth/csrf')).json()).data;const key=crypto.randomUUID();const body={expectedPlanVersion:p.planVersion,expectedPlanHash:p.planHash};const url='/api/voice/commands/'+p.proposalId+'/confirm';const opts={method:'POST',headers:{'Content-Type':'application/json',[csrf.headerName]:csrf.token,'Idempotency-Key':key},body:JSON.stringify(body)};const first=await fetch(url,opts);const a={status:first.status,body:await first.json()};const second=await fetch(url,opts);const b={status:second.status,body:await second.json()};return {first:a,second:b,key}},original);
 assert.equal(repeated.first.status,202);assert.equal(repeated.second.status,200);assert.equal(repeated.first.body.data.execution.executionId,repeated.second.body.data.execution.executionId);assert.equal(repeated.first.body.data.execution.commandId,repeated.second.body.data.execution.commandId);
 checks.push({caseId:'I08',result:'PASS',...repeated});
