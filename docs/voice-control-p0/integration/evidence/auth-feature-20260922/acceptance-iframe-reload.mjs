...
try {
 ...
 await page.locator('iframe[src*="unity-virtual-fleet"]').evaluate(f=>f.contentWindow.location.reload());
 await readyAgain;
 ...
 assert.notEqual(oldPayload.unityInstanceId,freshPayload.unityInstanceId);assert.notEqual(oldPayload.bindingId,freshPayload.bindingId);assert(freshPayload.sceneRevision>=1);
 ...
 assert.equal(network.slice(start).filter(n=>n.url.endsWith('/prepare')).length,0);
 checks.push({caseId:'X07/C05',result:'PASS',old:oldPayload,fresh:freshPayload,oldReportHttpCount:0,reprepareCount:0});
 await page.screenshot({path:output+'/iframe-reloaded.png',timeout:20000});
 ...
 await fs.writeFile(output+'/browser-iframe-reload.json',JSON.stringify({checks,failure,network},null,2));await browser.close();
}
