...
try {
 ...
 const prep=network.find(n=>n.url.endsWith('/prepare')&&n.status===200);const run=prep.url.match(/algorithm-runs\/(\d+)/)[1];
 const manual=await page.evaluate(async run=>{const c=(await(await fetch('/api/auth/csrf')).json()).data;const r=await fetch('/api/algorithm-runs/'+run+'/start',{method:'POST',headers:{'Content-Type':'application/json',[c.headerName]:c.token},body:'{}'});return {status:r.status,body:await r.json()}},run);assert.equal(manual.status,200);await page.waitForTimeout(1500);
 ...
 const stale=await page.evaluate(async p=>{const c=(await(await fetch('/api/auth/csrf')).json()).data;const body={expectedPlanVersion:p.planVersion,expectedPlanHash:p.planHash};const r=await fetch('/api/voice/commands/'+p.proposalId+'/confirm',{method:'POST',headers:{'Content-Type':'application/json',[c.headerName]:c.token,'Idempotency-Key':crypto.randomUUID()},body:JSON.stringify(body)});return {status:r.status,body:await r.json(),confirmation:body}},original);
 assert.equal(stale.status,409);assert.equal(stale.body.code,'CONTEXT_CHANGED');
 const proposals=network.filter(n=>n.url.endsWith('/commands/proposals')&&n.status<300);assert(proposals.every(n=>n.body.data.proposalId===original.proposalId));
 checks.push({caseId:'X16',result:'PASS',original,recoveredBody,manual,stale,proposalIds:proposals.map(n=>n.body.data.proposalId)});
 await page.screenshot({path:output+'/stale-proposal-recovery.png',timeout:20000});
 ...
 await fs.writeFile(output+'/browser-stale-proposal.json',JSON.stringify({checks,failure,network},null,2));await browser.close();
}
