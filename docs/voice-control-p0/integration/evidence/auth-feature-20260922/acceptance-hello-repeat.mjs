...
await page.addInitScript(()=>{
 ...
 window.__droppedHellos=0;window.__lastHello=null;window.__sceneLoads=0;
 window.addEventListener('message',e=>{let d=e.data;try{if(typeof d==='string')d=JSON.parse(d)}catch{return}const m=d?.message??d;if(m?.type==='loadScenario')window.__sceneLoads++;if(m?.type==='PRESENTATION_HELLO')window.__lastHello=e.data;if(m?.type==='PRESENTATION_HELLO'&&window.__droppedHellos<3){window.__droppedHellos++;e.stopImmediatePropagation();}},true);
});
const diagnostics=[];
...
try {
 ...
 const dropped=await page.locator('iframe[src*="unity-virtual-fleet"]').evaluate(f=>f.contentWindow.__droppedHellos);
 assert.equal(dropped,3);assert(network.some(n=>n.url.endsWith('/presentation/reports')&&n.status===200&&n.request?.kind==='SCENE_READY'));
 const before=await page.evaluate(()=>({message:window.__p0Messages.filter(m=>m.type==='PRESENTATION_REPORT').at(-1),loads:document.querySelector('iframe[src*=\"unity-virtual-fleet\"]').contentWindow.__sceneLoads}));
 const start=network.length;await page.locator('iframe[src*="unity-virtual-fleet"]').evaluate(f=>{for(let i=0;i<10;i++)f.contentWindow.postMessage(f.contentWindow.__lastHello,location.origin)});
 await page.waitForTimeout(5000);
 const after=await page.evaluate(()=>({message:window.__p0Messages.filter(m=>m.type==='PRESENTATION_REPORT').at(-1),loads:document.querySelector('iframe[src*=\"unity-virtual-fleet\"]').contentWindow.__sceneLoads}));
 const b=before.message.payload??before.message,a=after.message.payload??after.message;assert.equal(a.unityInstanceId,b.unityInstanceId);assert.equal(a.bindingId,b.bindingId);assert.equal(a.sceneRevision,b.sceneRevision);assert.equal(before.loads,after.loads);assert.equal(network.slice(start).filter(n=>n.url.endsWith('/prepare')).length,0);
 checks.push({duplicateHelloCount:10,before,after});
 checks.push({caseId:'X05',result:'PASS',droppedRealHelloMessages:dropped});
 await page.screenshot({path:output+'/hello-retry.png',timeout:20000});
 ...
 await fs.writeFile(output+'/browser-hello-retry.json',JSON.stringify({checks,failure,network},null,2));await browser.close();
}
