...
try {
 ...
 await page.goto('http://127.0.0.1:15174/');await page.waitForTimeout(1200);await page.goto('http://127.0.0.1:15174/?workspace=simulation');await click('语音控制');
 await readyAgain;
 ...
 checks.push({caseId:'C05',result:'PASS',old:oldPayload,fresh:freshPayload,oldReportHttpCount:0});
 await page.screenshot({path:output+'/page-return.png',timeout:20000});
 ...
 await fs.writeFile(output+'/browser-page-return.json',JSON.stringify({checks,failure,network},null,2));await browser.close();
}
