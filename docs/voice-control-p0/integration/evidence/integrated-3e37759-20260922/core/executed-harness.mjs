...
const dir=new URL('./acceptance-dual2-3e37759/',import.meta.url);await fs.mkdir(dir,{recursive:true});
...
try {
 ...
 await action('开始任务');await textWait('Unity 已应用');await record('start-applied');
 dropConfirm=true;await action('暂停任务');await textWait('上次写请求结果未知');await record('pause-response-lost');await page.waitForTimeout(32000);
 await page.reload();await panel();await textWait('暂停任务\n算法结果：SUCCESS');await record('pause-recovered');
 blockFrames=true;await action('继续任务');await textWait('Unity 同步中');await record('resume-pending');await textWait('Unity 展示已过期');await record('resume-stale');await page.screenshot({path:new URL('stale.png',dir).pathname.replace(/^\/(\w:)/,'$1'),timeout:20000});
 blockFrames=false;await click('重新同步画面');await textWait('Unity 已应用');await record('resume-reported');
 await page.reload();await panel();await page.getByRole('button',{name:'停止任务 创建并核对指令提案',exact:true}).waitFor({timeout:60000});await record('running-reloaded');
 await action('停止任务');await textWait('停止任务\n算法结果：SUCCESS');await record('stop-succeeded');
 await page.reload();await panel();await textWait('停止任务\n算法结果：SUCCESS');await page.waitForTimeout(6000);await record('stop-reloaded');await page.screenshot({path:new URL('stopped.png',dir).pathname.replace(/^\/(\w:)/,'$1')});
 ...
 async function signIn(user,password){await page.getByPlaceholder('请输入用户名').fill(user);await page.getByPlaceholder('请输入密码').fill(password);await click('进入控制台');await panel();await page.getByRole('button',{name:'停止任务 创建并核对指令提案',exact:true}).waitFor({timeout:90000});}
 ...
 await rejectOther('p0_admin',aRef,aExec,aProposal);await record('peer-isolated');
 ...
 await record('owner-restored-without-peer-data');
